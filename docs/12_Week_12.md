...
In this part of the lab section, we will introduce non-linear (type II) functional response into the consumer-resource model in Part 1. for one of the consumers and change the resource equation to logistic growth. We also allow the two consumers to have different conversion efficiencies and mortality rates:
