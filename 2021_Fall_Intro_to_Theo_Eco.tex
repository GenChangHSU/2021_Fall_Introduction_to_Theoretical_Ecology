...
\item
  ...
  \item
    Equilibrium \(N^*\): fixed point; good candidates where the system will end up \(\frac{dN}{dt} = f_{(N^*)} = 0\)
  \item
    For logistic growth equation: \(\frac{dN}{dt} = r_{0}N(1-\frac{N}{K})\); \(N^* = 0, K\)
  \item
...
\item
  ...
  \tightlist
  \item
    Plot the function \(\frac{dN}{dt} = f(N)\) and determine the direction of change (positive/negative) on both sides of the equilibrium points \(N_{0}\)
  \end{enumerate}
\end{itemize}
...
\item
  ...
  \item
    Examine how \(\epsilon_{(t)}\) changes over time (i.e., the behavior of the small displacement)
  \item
  \item
